    ...
    public static void register() {
        ServerPlayNetworking.registerGlobalReceiver(SYNC_CHANNEL, (server, player, handler, buf, responseSender) -> {
            ...
            server.execute(() -> {
                ...
                sendCurrentRange(player, levelFor(player));
            });
        });

        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> sendCurrentRange(handler.player, levelFor(handler.player)));
