...
public final class WorldHeightScreenHooks {
    ...
    private static void addWidget(CreateWorldScreen screen, AbstractWidget widget) {
        try {
            ...
            return;
        } catch (ClassCastException | AbstractMethodError ignored) {
            // Fallback if the mixin didn't apply in this environment.
        }
        ...
        try {
            if (ADD_WIDGET_METHOD == null) {
                for (Method method : Screen.class.getDeclaredMethods()) {
                    ...
                    if (AbstractWidget.class.isAssignableFrom(param) || param.isAssignableFrom(AbstractWidget.class)) {
            ...
            ADD_WIDGET_METHOD.invoke(screen, widget);
        } catch (ReflectiveOperationException ex) {
            throw new IllegalStateException("Unable to add custom widget to CreateWorldScreen", ex);
        }
    }
