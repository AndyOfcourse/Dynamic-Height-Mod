...
public final class HeightValidator {
    ...
    public static final int ABS_MAX_Y = 2015;
    public static final int MIN_UPPER_BOUND = -64;
    public static final int MAX_LOWER_BOUND = 320;
    ...
    public static int sanitizeMin(int value) {
        int maxMin = Math.min(MIN_UPPER_BOUND, ABS_MAX_Y - (CHUNK_ALIGNMENT - 1));
        int clamped = Math.max(ABS_MIN_Y, Math.min(value, maxMin));
        // DimensionType requires minY to be a multiple of 16. Snap down to avoid jitter.
        int snapped = snapDownToMultiple(clamped, CHUNK_ALIGNMENT);
        if (snapped > maxMin) {
    ...
    public static int sanitizeMax(int min, int value) {
        ...
        int minAllowed = Math.max(min + (CHUNK_ALIGNMENT - 1), MAX_LOWER_BOUND);
        int clamped = Math.max(minAllowed, Math.min(value, maxAllowed));

        // Height must be a multiple of 16, so top Y = min + (k * 16) - 1.
        int height = snapUpToMultiple((clamped - min) + 1, CHUNK_ALIGNMENT);
