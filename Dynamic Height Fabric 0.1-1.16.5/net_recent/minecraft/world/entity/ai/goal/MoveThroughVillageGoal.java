      ...
      if (!GoalUtils.hasGroundPathNavigation(this.mob)) {
         ...
         if (this.onlyAtNight && this.mob.level.isDay()) {
            ...
            if (!serverLevel.isCloseToVillage(blockPos, 6)) {
               ...
               Vec3 vec3 = RandomPos.getLandPos(
                  this.mob,
                  15,
                  7,
                  blockPos2x -> {
                     if (!serverLevel.isVillage(blockPos2x)) {
                        ...
                        Optional<BlockPos> optionalxx = serverLevel.getPoiManager()
                           .find(PoiType.ALL, this::hasNotVisited, blockPos2x, 10, PoiManager.Occupancy.IS_OCCUPIED);
                        return !optionalxx.isPresent() ? Double.NEGATIVE_INFINITY : -optionalxx.get().distSqr(blockPos);
                     }
                  }
               );
               if (vec3 == null) {
