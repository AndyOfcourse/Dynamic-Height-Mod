         ...
         if (this.ticksSinceReachedGoal > 60) {
            ...
            if (!level.isClientSide) {
               for(int i = 0; i < 20; ++i) {
                  ...
                  ((ServerLevel)level)
                     .sendParticles(
                        ParticleTypes.POOF, (double)blockPos2.getX() + 0.5, (double)blockPos2.getY(), (double)blockPos2.getZ() + 0.5, 1, d, e, f, 0.15F
                     );
               }
   ...
   private BlockPos getPosWithBlock(BlockPos blockPos, BlockGetter blockGetter) {
      if (blockGetter.getBlockState(blockPos).is(this.blockToRemove)) {
         ...
         BlockPos[] blockPoss = new BlockPos[]{
            blockPos.below(), blockPos.west(), blockPos.east(), blockPos.north(), blockPos.south(), blockPos.below().below()
         };
