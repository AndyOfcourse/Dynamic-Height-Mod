...
public class KeyboardHandler {
   ...
   public void keyPress(long l, int i, int j, int k, int m) {
      if (l == this.minecraft.getWindow().getWindow()) {
         ...
         if (k == 1
            && (!(this.minecraft.screen instanceof ControlsScreen) || ((ControlsScreen)containerEventHandler).lastKeySelection <= Util.getMillis() - 20L)) {
   ...
   public void setup(long l) {
      InputConstants.setupKeyboardCallbacks(
         l,
         (lx, i, j, k, m) -> this.minecraft.execute(() -> this.keyPress(lx, i, j, k, m)),
         (lx, i, j) -> this.minecraft.execute(() -> this.charTyped(lx, i, j))
      );
   }
