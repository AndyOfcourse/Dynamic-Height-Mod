...
public class ClientRecipeBook extends RecipeBook {
      ...
      RecipeBookCategories.AGGREGATE_CATEGORIES
         .forEach(
            (recipeBookCategories, list) -> list.stream()
                  .flatMap(recipeBookCategoriesx -> map2.getOrDefault(recipeBookCategoriesx, ImmutableList.of()).stream())
         );
   ...
   private static Map<RecipeBookCategories, List<List<Recipe<?>>>> categorizeAndGroupRecipes(Iterable<Recipe<?>> iterable) {
      ...
      for(Recipe<?> recipe : iterable) {
         if (!recipe.isSpecial()) {
            ...
            if (string.isEmpty()) {
               map.computeIfAbsent(recipeBookCategories, recipeBookCategoriesx -> Lists.newArrayList()).add(ImmutableList.of(recipe));
            } else {
               List<Recipe<?>> list = (List)table.get(recipeBookCategories, string);
               if (list == null) {
                  list = Lists.newArrayList();
                  table.put(recipeBookCategories, string, list);
                  map.computeIfAbsent(recipeBookCategories, recipeBookCategoriesx -> Lists.newArrayList()).add(list);
               }
   ...
   private static RecipeBookCategories getCategory(Recipe<?> recipe) {
      ...
      if (recipeType == RecipeType.CRAFTING) {
         ...
         LOGGER.warn("Unknown recipe category: {}/{}", new Supplier[]{() -> Registry.RECIPE_TYPE.getKey(recipe.getType()), recipe::getId});
