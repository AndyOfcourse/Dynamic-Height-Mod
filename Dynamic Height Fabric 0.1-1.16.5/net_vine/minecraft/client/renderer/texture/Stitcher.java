...
public class Stitcher {
   private static final Comparator<Stitcher.Holder> HOLDER_COMPARATOR = Comparator.<Stitcher.Holder, Integer>comparing(holder -> -holder.height)
   ...
   public void stitch() {
      ...
      for(Stitcher.Holder holder : list) {
         if (!this.addToStorage(holder)) {
            throw new StitcherException(holder.spriteInfo, list.stream().map(holderx -> holderx.spriteInfo).collect(ImmutableList.toImmutableList()));
         }
      }
