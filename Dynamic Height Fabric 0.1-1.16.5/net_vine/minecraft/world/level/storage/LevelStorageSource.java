...
public class LevelStorageSource {
   ...
   private static <T> Pair<WorldGenSettings, Lifecycle> readWorldGenSettings(Dynamic<T> dynamic, DataFixer dataFixer, int i) {
      ...
      return Pair.of(
         dataResult.resultOrPartial(Util.prefix("WorldGenSettings: ", LOGGER::error))
            .orElseGet(
               () -> {
   ...
   private <T> T readLevelData(File file, BiFunction<File, DataFixer, T> biFunction) {
      if (!file.exists()) {
         ...
         if (file2.exists()) {
            T object = biFunction.apply(file2, this.fixerUpper);
            if (object != null) {
               return object;
            }
         }
         ...
         return file2.exists() ? biFunction.apply(file2, this.fixerUpper) : null;
      }
   }
      ...
      return (file, dataFixer) -> {
         try {
            ...
            return PrimaryLevelData.parse(
               dynamic, dataFixer, i, compoundTag3, levelSettings, levelVersion, (WorldGenSettings)pair.getFirst(), (Lifecycle)pair.getSecond()
            );
   ...
   public boolean levelExists(String string) {
      return Files.isDirectory(this.baseDir.resolve(string));
   }
   ...
   public class LevelStorageAccess implements AutoCloseable {
      ...
      public Path getLevelPath(LevelResource levelResource) {
         return this.resources.computeIfAbsent(levelResource, levelResourcex -> this.levelPath.resolve(levelResourcex.getId()));
      }
