...
public class RecipeManager extends SimpleJsonResourceReloadListener {
   ...
   protected void apply(Map<ResourceLocation, JsonElement> map, ResourceManager resourceManager, ProfilerFiller profilerFiller) {
      ...
      Map<RecipeType<?>, Builder<ResourceLocation, Recipe<?>>> map2 = Maps.newHashMap();
   ...
   public <C extends Container, T extends Recipe<C>> List<T> getAllRecipesFor(RecipeType<T> recipeType) {
      return this.byType(recipeType).values().stream().map((Function<? super Recipe, ? extends Recipe>)(recipe -> recipe)).collect(Collectors.toList());
   }
