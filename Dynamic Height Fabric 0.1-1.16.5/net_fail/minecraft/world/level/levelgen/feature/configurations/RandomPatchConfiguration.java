...
public class RandomPatchConfiguration implements FeatureConfiguration {
   public static final Codec<RandomPatchConfiguration> CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
               ...
               BlockState.CODEC
                  .listOf()
                  .fieldOf("whitelist")
                  .forGetter(randomPatchConfiguration -> randomPatchConfiguration.whitelist.stream().map(Block::defaultBlockState).collect(Collectors.toList())),
   ...
   private RandomPatchConfiguration(
      ...
      this(
         blockStateProvider,
         blockPlacer,
         list.stream().map(BlockBehaviour.BlockStateBase::getBlock).collect(Collectors.toSet()),
