...
public class PlayerAdvancements {
   ...
   public PlayerAdvancements(
      DataFixer dataFixer, PlayerList playerList, ServerAdvancementManager serverAdvancementManager, File file, ServerPlayer serverPlayer
   ) {
         ...
         try {
            ...
            try {
               ...
               dynamic = this.dataFixer
                  .update(
                     DataFixTypes.ADVANCEMENTS.getType(), dynamic, dynamic.get("DataVersion").asInt(0), SharedConstants.getCurrentVersion().getWorldVersion()
                  );
               dynamic = dynamic.remove("DataVersion");
               Map<ResourceLocation, AdvancementProgress> map = (Map)GSON.getAdapter(TYPE_TOKEN).fromJsonTree((JsonElement)dynamic.getValue());
               if (map == null) {
