...
public class Commands {
         ...
         try {
            ...
            if (var14.getInput() != null && var14.getCursor() >= 0) {
               ...
               if (i < var14.getInput().length()) {
                  Component component = new TextComponent(var14.getInput().substring(i))
                     .withStyle(new ChatFormatting[]{ChatFormatting.RED, ChatFormatting.UNDERLINE});
                  mutableComponent.append(component);
               }

               mutableComponent.append(
                  new TranslatableComponent("command.context.here").withStyle(new ChatFormatting[]{ChatFormatting.RED, ChatFormatting.ITALIC})
               );
            ...
            commandSourceStack.sendFailure(
               new TranslatableComponent("command.failed")
                  .withStyle(style -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, mutableComponent2)))
            );
            if (SharedConstants.IS_RUNNING_IN_IDE) {
   ...
   public static void validate() {
      ...
      Set<ArgumentType<?>> set2 = set.stream().filter(argumentType -> !ArgumentTypes.isTypeRegistered(argumentType)).collect(Collectors.toSet());
      if (!set2.isEmpty()) {
         LOGGER.warn(
