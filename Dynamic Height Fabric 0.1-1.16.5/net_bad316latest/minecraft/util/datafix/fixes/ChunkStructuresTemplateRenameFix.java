...
public class ChunkStructuresTemplateRenameFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped(
         ...
         typed -> typed.updateTyped(
               opticFinder, typed2 -> typed2.update(DSL.remainderFinder(), dynamic -> this.fixTag((Dynamic<?>)typed.get(DSL.remainderFinder()), dynamic))
            )
      );
   }
   ...
   private Dynamic<?> fixTag(Dynamic<?> dynamic, Dynamic<?> dynamic2) {
      ...
      if (RENAMES.containsKey(string)) {
         Pair<String, ImmutableMap<String, String>> pair = (Pair)RENAMES.get(string);
