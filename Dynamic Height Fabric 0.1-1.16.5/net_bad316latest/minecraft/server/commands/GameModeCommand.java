...
public class GameModeCommand {
   public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
      LiteralArgumentBuilder<CommandSourceStack> literalArgumentBuilder = (LiteralArgumentBuilder)Commands.literal("gamemode")
         .requires(commandSourceStack -> commandSourceStack.hasPermission(2));

      for(GameType gameType : GameType.values()) {
         if (gameType != GameType.NOT_SET) {
            literalArgumentBuilder.then(
               ((LiteralArgumentBuilder)Commands.literal(gameType.getName())
                     .executes(
                        commandContext -> setMode(
                              commandContext, Collections.singleton(((CommandSourceStack)commandContext.getSource()).getPlayerOrException()), gameType
                           )
                     ))
                  .then(
