...
public class LocateCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
      LiteralArgumentBuilder<CommandSourceStack> literalArgumentBuilder = (LiteralArgumentBuilder)Commands.literal("locate")
      ...
      for(Entry<String, StructureFeature<?>> entry : StructureFeature.STRUCTURES_REGISTRY.entrySet()) {
         literalArgumentBuilder = (LiteralArgumentBuilder)literalArgumentBuilder.then(
            Commands.literal(entry.getKey()).executes(commandContext -> locate((CommandSourceStack)commandContext.getSource(), entry.getValue()))
         );
      }
