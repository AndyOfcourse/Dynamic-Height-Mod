            ...
            processorHandle2 -> new StrictQueue.IntRunnable(
                  ...
                  () -> {
                     ...
                     processorHandle2.tell(
                        ProcessorHandle.of(
                           "chunk priority sorter around " + processorHandle.name(),
                           message -> this.submit(processorHandle, message.task, message.pos, message.level, bl)
                        )
                     );
                  }
               )
         ...
         .<ProcessorHandle<ChunkTaskPriorityQueueSorter.Release>>ask(
            processorHandle2 -> new StrictQueue.IntRunnable(
                  ...
                  () -> processorHandle2.tell(
                        ProcessorHandle.of(
                           "chunk priority sorter around " + processorHandle.name(),
                           release -> this.release(processorHandle, release.pos, release.task, release.clearQueue)
                        )
                     )
               )
         )
   ...
   private <T> void pollTask(ChunkTaskPriorityQueue<Function<ProcessorHandle<Unit>, T>> chunkTaskPriorityQueue, ProcessorHandle<T> processorHandle) {
      this.mailbox.tell(new StrictQueue.IntRunnable(3, () -> {
         ...
         if (stream == null) {
            ...
            Util.sequence(stream.<CompletableFuture>map(either -> (CompletableFuture)either.map(processorHandle::ask, runnable -> {
                  runnable.run();
      ...
      return (String)this.queues
            ...
            .map(
               entry -> entry.getKey().name()
                     + "=["
                     + (String)entry.getValue().getAcquired().stream().map(long_ -> long_ + ":" + new ChunkPos(long_)).collect(Collectors.joining(","))
                     + "]"
            )
