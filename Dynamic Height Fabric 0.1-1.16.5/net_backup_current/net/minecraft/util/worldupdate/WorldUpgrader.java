...
public class WorldUpgrader {
   ...
   public WorldUpgrader(
      LevelStorageSource.LevelStorageAccess levelStorageAccess, DataFixer dataFixer, ImmutableSet<ResourceKey<Level>> immutableSet, boolean bl
   ) {
   ...
   private void work() {
      ...
      List<ChunkPos> list;
      for(UnmodifiableIterator f = this.levels.iterator(); f.hasNext(); this.totalChunks += list.size()) {
         ResourceKey<Level> resourceKey = (ResourceKey)f.next();
      ...
      if (this.totalChunks == 0) {
         ...
         UnmodifiableIterator immutableMap2 = this.levels.iterator();

         while(immutableMap2.hasNext()) {
            ResourceKey<Level> resourceKey2 = (ResourceKey)immutableMap2.next();
         ...
         ImmutableMap<ResourceKey<Level>, ChunkStorage> immutableMap2x = builder2.build();
         ...
         while(this.running) {
            ...
            float h;
            for(UnmodifiableIterator iOException2 = this.levels.iterator(); iOException2.hasNext(); g += h) {
               ...
               ChunkStorage chunkStorage = (ChunkStorage)immutableMap2x.get(resourceKey3);
