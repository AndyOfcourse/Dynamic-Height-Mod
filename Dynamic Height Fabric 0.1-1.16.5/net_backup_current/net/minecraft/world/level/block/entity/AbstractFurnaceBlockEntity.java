...
public abstract class AbstractFurnaceBlockEntity
   ...
   public void tick() {
      ...
      if (!this.level.isClientSide) {
         ...
         if (bl != this.isLit()) {
            bl2 = true;
            this.level
               .setBlock(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(AbstractFurnaceBlock.LIT, Boolean.valueOf(this.isLit())), 3);
         }
      }
   ...
   public List<Recipe<?>> getRecipesToAwardAndPopExperience(Level level, Vec3 vec3) {
      ...
      while(var4.hasNext()) {
         Entry<ResourceLocation> entry = (Entry)var4.next();
