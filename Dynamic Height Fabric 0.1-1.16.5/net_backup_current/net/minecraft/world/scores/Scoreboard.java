...
public class Scoreboard {
   ...
   public Objective addObjective(String string, ObjectiveCriteria objectiveCriteria, Component component, ObjectiveCriteria.RenderType renderType) {
      if (string.length() > 16) {
         ...
         this.objectivesByCriteria.computeIfAbsent(objectiveCriteria, objectiveCriteriax -> Lists.newArrayList()).add(objective);
   ...
   public Score getOrCreatePlayerScore(String string, Objective objective) {
      if (string.length() > 40) {
         ...
         Map<Objective, Score> map = this.playerScores.computeIfAbsent(string, stringx -> Maps.newHashMap());
