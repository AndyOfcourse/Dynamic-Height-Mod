...
public abstract class Language {
      ...
      return new Language() {
         ...
         public FormattedCharSequence getVisualOrder(FormattedText formattedText) {
            return formattedCharSink -> formattedText.visit(
                     (style, string) -> StringDecomposer.iterateFormatted(string, style, formattedCharSink) ? Optional.empty() : FormattedText.STOP_ITERATION,
                     Style.EMPTY
                  )
                  .isPresent();
         }
   ...
   public static void loadFromJson(InputStream inputStream, BiConsumer<String, String> biConsumer) {
      ...
      for(Entry<String, JsonElement> entry : jsonObject.entrySet()) {
         String string = UNSUPPORTED_FORMAT_PATTERN.matcher(GsonHelper.convertToString((JsonElement)entry.getValue(), entry.getKey())).replaceAll("%$1s");
   ...
   public List<FormattedCharSequence> getVisualOrder(List<FormattedText> list) {
      return list.stream().map(getInstance()::getVisualOrder).collect(ImmutableList.toImmutableList());
   }
}
