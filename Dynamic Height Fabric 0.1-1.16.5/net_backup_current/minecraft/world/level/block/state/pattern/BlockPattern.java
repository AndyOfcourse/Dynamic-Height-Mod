...
public class BlockPattern {
      ...
      for(int i = 0; i < this.width; ++i) {
         for(int j = 0; j < this.height; ++j) {
            for(int k = 0; k < this.depth; ++k) {
               if (!this.pattern[k][j][i].test((BlockInWorld)loadingCache.getUnchecked(translateAndRotate(blockPos, direction, direction2, i, j, k)))) {
                  return null;
               }
            }
         }
   ...
   public static class BlockPatternMatch {
      ...
      public BlockInWorld getBlock(int i, int j, int k) {
         return (BlockInWorld)this.cache.getUnchecked(BlockPattern.translateAndRotate(this.frontTopLeft, this.getForwards(), this.getUp(), i, j, k));
      }
