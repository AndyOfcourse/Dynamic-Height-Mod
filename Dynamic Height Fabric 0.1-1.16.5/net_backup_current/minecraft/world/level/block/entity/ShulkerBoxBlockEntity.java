...
public class ShulkerBoxBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer, TickableBlockEntity {
   ...
   public AABB getBoundingBox(Direction direction) {
      ...
      return Shapes.block()
         .bounds()
         .expandTowards(
            (double)(0.5F * f * (float)direction.getStepX()),
            (double)(0.5F * f * (float)direction.getStepY()),
            (double)(0.5F * f * (float)direction.getStepZ())
         );
   }
         ...
         if (!list.isEmpty()) {
            for(int i = 0; i < list.size(); ++i) {
               ...
               if (entity.getPistonPushReaction() != PushReaction.IGNORE) {
                  ...
                  entity.move(
                     MoverType.SHULKER_BOX, new Vec3(d * (double)direction.getStepX(), e * (double)direction.getStepY(), f * (double)direction.getStepZ())
                  );
               }
            }
         }
