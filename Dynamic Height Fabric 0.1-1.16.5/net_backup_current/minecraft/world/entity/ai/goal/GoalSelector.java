...
public class GoalSelector {
   ...
   public void tick() {
      ...
      this.getRunningGoals()
         .filter(
            wrappedGoal -> !wrappedGoal.isRunning()
                  || wrappedGoal.getFlags().stream().anyMatch(this.disabledFlags::contains)
                  || !wrappedGoal.canContinueToUse()
         )
         .forEach(Goal::stop);
      ...
      this.availableGoals
         ...
         .filter(wrappedGoal -> wrappedGoal.getFlags().stream().allMatch(flag -> this.lockedFlags.getOrDefault(flag, NO_GOAL).canBeReplacedBy(wrappedGoal)))
