...
public interface Tag<T> {
   static <T> Codec<Tag<T>> codec(Supplier<TagCollection<T>> supplier) {
      return ResourceLocation.CODEC
         .flatXmap(
            resourceLocation -> (DataResult)Optional.ofNullable(supplier.get().getTag(resourceLocation))
                  .map(DataResult::success)
                  .orElseGet(() -> (T)DataResult.error("Unknown tag: " + resourceLocation)),
            tag -> (DataResult)Optional.ofNullable(supplier.get().getId(tag))
                  .map(DataResult::success)
                  .orElseGet(() -> (T)DataResult.error("Unknown tag: " + tag))
         );
   }
