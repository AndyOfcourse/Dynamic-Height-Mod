...
public class MappedRegistry<T> extends WritableRegistry<T> {
      ...
      return RecordCodecBuilder.mapCodec(
         instance -> instance.group(
                  ResourceLocation.CODEC
                     .xmap(ResourceKey.elementKey(resourceKey), ResourceKey::location)
                     .fieldOf("name")
                     .forGetter(registryEntry -> registryEntry.key),
   ...
   private <V extends T> V registerMapping(int i, ResourceKey<T> resourceKey, V object, Lifecycle lifecycle, boolean bl) {
      ...
      this.lifecycles.put((T)object, (V)lifecycle);
   ...
   public Optional<ResourceKey<T>> getResourceKey(T object) {
      return Optional.ofNullable((ResourceKey<T>)this.keyStorage.inverse().get(object));
   }
   ...
   public Set<Entry<ResourceKey<T>, T>> entrySet() {
      return Collections.<ResourceKey<T>, T>unmodifiableMap(this.keyStorage).entrySet();
   }
