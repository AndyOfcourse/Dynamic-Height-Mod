...
public class MessageArgument implements ArgumentType<MessageArgument.Message> {
   ...
   public static Component getMessage(CommandContext<CommandSourceStack> commandContext, String string) throws CommandSyntaxException {
      return ((MessageArgument.Message)commandContext.getArgument(string, MessageArgument.Message.class))
         .toComponent((CommandSourceStack)commandContext.getSource(), ((CommandSourceStack)commandContext.getSource()).hasPermission(2));
   }
            ...
            while(true) {
               ...
               while(true) {
                  if (!stringReader.canRead()) {
                     return new MessageArgument.Message(string, list.toArray(new MessageArgument.Part[list.size()]));
                  }
