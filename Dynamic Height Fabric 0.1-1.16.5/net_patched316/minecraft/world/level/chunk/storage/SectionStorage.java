      ...
      if (object == null) {
         ...
         for(int l = 0; l < 16; ++l) {
            ...
            Optional<R> optional = optionalDynamic.get(Integer.toString(l))
               .result()
               .flatMap(dynamicx -> ((Codec)this.codec.apply(() -> this.setDirty(m))).parse(dynamicx).resultOrPartial(LOGGER::error));
   ...
   private <T> Dynamic<T> writeColumn(ChunkPos chunkPos, DynamicOps<T> dynamicOps) {
      ...
      for(int i = 0; i < 16; ++i) {
         ...
         if (optional != null && optional.isPresent()) {
            DataResult<T> dataResult = ((Codec)this.codec.apply(() -> this.setDirty(l))).encodeStart(dynamicOps, optional.get());
