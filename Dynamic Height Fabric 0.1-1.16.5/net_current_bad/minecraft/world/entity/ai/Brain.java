...
public class Brain<E extends LivingEntity> {
         ...
         (new MapCodec<Brain<E>>() {
               public <T> Stream<T> keys(DynamicOps<T> dynamicOps) {
                  return collection.stream()
                     ...
                     .map(resourceLocation -> (T)dynamicOps.createString(resourceLocation.toString()));
               }
               ...
               public <T> DataResult<Brain<E>> decode(DynamicOps<T> dynamicOps, MapLike<T> mapLike) {
                  ...
                  ImmutableList<Brain.MemoryValue<?>> immutableList = (ImmutableList)((DataResult)mutableObjectx.getValue())
               ...
               private <T, U> DataResult<Brain.MemoryValue<U>> captureRead(MemoryModuleType<U> memoryModuleType, DynamicOps<T> dynamicOps, T object) {
                  return ((DataResult)memoryModuleType.getCodec()
                        ...
                        .orElseGet(() -> (T)DataResult.error("No codec for memory: " + memoryModuleType)))
   ...
   public Brain(
      ...
      UnmodifiableIterator var11 = immutableList.iterator();
   ...
   private Stream<Brain.MemoryValue<?>> memories() {
      return this.memories
         .entrySet()
         .stream()
         .map(entry -> Brain.MemoryValue.createUnchecked((MemoryModuleType<? extends ExpirableValue<?>>)entry.getKey(), entry.getValue()));
   }
   ...
   private void addActivityAndRemoveMemoriesWhenStopped(
      ...
      while(var5.hasNext()) {
         ...
         this.availableBehaviorsByPriority
            ...
            .add((Behavior<? super E>)pair.getSecond());
      }
   }
               ...
               codec -> this.value
                     .ifPresent(
                        expirableValue -> recordBuilder.add(
                              Registry.MEMORY_MODULE_TYPE.encodeStart(dynamicOps, this.type), codec.encodeStart(dynamicOps, expirableValue)
                           )
                     )
            );
      }
