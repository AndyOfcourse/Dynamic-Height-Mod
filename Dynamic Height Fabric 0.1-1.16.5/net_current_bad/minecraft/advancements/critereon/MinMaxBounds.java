...
public abstract class MinMaxBounds<T extends Number> {
   ...
   protected static <T extends Number, R extends MinMaxBounds<T>> R fromJson(
      ...
      if (jsonElement == null || jsonElement.isJsonNull()) {
         ...
         T number = biFunction.apply((T)jsonElement, "value");
         return boundsFactory.create(number, number);
      } else {
         JsonObject jsonObject = GsonHelper.convertToJsonObject(jsonElement, "value");
         T number2 = jsonObject.has("min") ? biFunction.apply((T)jsonObject.get("min"), "min") : null;
         T number3 = jsonObject.has("max") ? biFunction.apply((T)jsonObject.get("max"), "max") : null;
   ...
   protected static <T extends Number, R extends MinMaxBounds<T>> R fromReader(
      ...
      if (!stringReader.canRead()) {
         ...
         try {
            T number = optionallyFormat(readNumber(stringReader, function, supplier), function2);
            T number2;
            if (stringReader.canRead(2) && stringReader.peek() == '.' && stringReader.peek(1) == '.') {
               stringReader.skip();
               stringReader.skip();
               number2 = optionallyFormat(readNumber(stringReader, function, supplier), function2);
   ...
   public static class Floats extends MinMaxBounds<Float> {
      ...
      public static MinMaxBounds.Floats fromReader(StringReader stringReader, Function<Float, Float> function) throws CommandSyntaxException {
         return fromReader(
            stringReader, MinMaxBounds.Floats::create, Float::parseFloat, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidFloat, function
         );
      }
   }
