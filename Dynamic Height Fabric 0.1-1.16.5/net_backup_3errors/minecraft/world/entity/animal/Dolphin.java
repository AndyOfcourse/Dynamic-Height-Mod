   ...
   static class DolphinSwimToTreasureGoal extends Goal {
      ...
      public void start() {
         if (this.dolphin.level instanceof ServerLevel) {
            ...
            if (blockPos2 == null) {
               StructureFeature<?> structureFeature2 = structureFeature.equals(StructureFeature.OCEAN_RUIN)
      ...
      private void drop(ItemStack itemStack) {
         if (!itemStack.isEmpty()) {
            ...
            itemEntity.setDeltaMovement(
               (double)(
                  0.3F * -Mth.sin(Dolphin.this.yRot * (float) (Math.PI / 180.0)) * Mth.cos(Dolphin.this.xRot * (float) (Math.PI / 180.0)) + Mth.cos(g) * h
               ),
