...
public class LevelFlatGeneratorInfoFix extends DataFix {
   ...
   private Dynamic<?> fix(Dynamic<?> dynamic) {
      return dynamic.get("generatorName").asString("").equalsIgnoreCase("flat")
         ? dynamic.update(
            "generatorOptions",
            dynamicx -> (Dynamic)DataFixUtils.orElse(dynamicx.asString().map(this::fixString).map(dynamicx::createString).result(), dynamicx)
         )
         : dynamic;
   }
   ...
   String fixString(String string) {
      if (string.isEmpty()) {
         ...
         if (i >= 0 && i <= 3) {
            ...
            stringBuilder.append(StreamSupport.<String>stream(LAYER_SPLITTER.split(string3).spliterator(), false).map(stringx -> {
               List<String> list = splitter.splitToList(stringx);
               int j;
               String string2xx;
               if (list.size() == 2) {
                  j = NumberUtils.toInt(list.get(0));
                  string2xx = list.get(1);
               } else {
                  j = 1;
                  string2xx = list.get(0);
               }

               List<String> list2 = BLOCK_SPLITTER.splitToList(string2xx);
               int k = list2.get(0).equals("minecraft") ? 1 : 0;
               String string3xx = list2.get(k);
