                                                      ...
                                                      Commands.argument("criterion", StringArgumentType.greedyString())
                                                         .suggests(
                                                            (commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggest(
                                                                  ResourceLocationArgument.getAdvancement(commandContext, "advancement").getCriteria().keySet(),
                                                                  suggestionsBuilder
                                                               )
                                                         )
                                                         .executes(
                                          ...
                                          Commands.argument("advancement", ResourceLocationArgument.id())
                                             ...
                                             .executes(
                                                commandContext -> perform(
                                                      ...
                                                      AdvancementCommands.Action.GRANT,
                                                      getAdvancements(
                                                         ResourceLocationArgument.getAdvancement(commandContext, "advancement"), AdvancementCommands.Mode.UNTIL
                                                      )
                                                   )
                                             )
                                       )
                                       ...
                                       Commands.argument("advancement", ResourceLocationArgument.id())
                                          ...
                                          .executes(
                                             commandContext -> perform(
                                                   ...
                                                   AdvancementCommands.Action.GRANT,
                                                   getAdvancements(
                                                      ResourceLocationArgument.getAdvancement(commandContext, "advancement"), AdvancementCommands.Mode.THROUGH
                                                   )
                                                )
                                          )
                                    )
                              ...
                              Commands.literal("everything")
                                 .executes(
                                    commandContext -> perform(
                                          ...
                                          EntityArgument.getPlayers(commandContext, "targets"),
                                          AdvancementCommands.Action.GRANT,
                                          ((CommandSourceStack)commandContext.getSource()).getServer().getAdvancements().getAllAdvancements()
                                       )
                                 )
                           )
                     )
                                          ...
                                          Commands.argument("advancement", ResourceLocationArgument.id())
                                             ...
                                             .executes(
                                                commandContext -> perform(
                                                      ...
                                                      AdvancementCommands.Action.REVOKE,
                                                      getAdvancements(
                                                         ResourceLocationArgument.getAdvancement(commandContext, "advancement"), AdvancementCommands.Mode.FROM
                                                      )
                                                   )
                                             )
                                       )
            ...
            if (collection.size() == 1) {
               throw new CommandRuntimeException(
                  new TranslatableComponent(
                     action.getKey() + ".one.to.one.failure", collection2.iterator().next().getChatComponent(), collection.iterator().next().getDisplayName()
                  )
               );
            } else {
               throw new CommandRuntimeException(
                  new TranslatableComponent(action.getKey() + ".one.to.many.failure", collection2.iterator().next().getChatComponent(), collection.size())
               );
            }
         } else if (collection.size() == 1) {
            throw new CommandRuntimeException(
               new TranslatableComponent(action.getKey() + ".many.to.one.failure", collection2.size(), collection.iterator().next().getDisplayName())
            );
         } else {
         ...
         if (collection2.size() == 1) {
            if (collection.size() == 1) {
               commandSourceStack.sendSuccess(
                  new TranslatableComponent(
                     action.getKey() + ".one.to.one.success", collection2.iterator().next().getChatComponent(), collection.iterator().next().getDisplayName()
                  ),
                  true
               );
            } else {
               commandSourceStack.sendSuccess(
                  new TranslatableComponent(action.getKey() + ".one.to.many.success", collection2.iterator().next().getChatComponent(), collection.size()),
                  true
               );
            }
         } else if (collection.size() == 1) {
            commandSourceStack.sendSuccess(
               new TranslatableComponent(action.getKey() + ".many.to.one.success", collection2.size(), collection.iterator().next().getDisplayName()), true
            );
         } else {
      ...
      if (!advancement.getCriteria().containsKey(string)) {
         ...
         if (i == 0) {
            if (collection.size() == 1) {
               throw new CommandRuntimeException(
                  new TranslatableComponent(
                     action.getKey() + ".criterion.to.one.failure", string, advancement.getChatComponent(), collection.iterator().next().getDisplayName()
                  )
               );
            } else {
               throw new CommandRuntimeException(
            ...
            if (collection.size() == 1) {
               commandSourceStack.sendSuccess(
                  new TranslatableComponent(
                     action.getKey() + ".criterion.to.one.success", string, advancement.getChatComponent(), collection.iterator().next().getDisplayName()
                  ),
                  true
               );
            } else {
