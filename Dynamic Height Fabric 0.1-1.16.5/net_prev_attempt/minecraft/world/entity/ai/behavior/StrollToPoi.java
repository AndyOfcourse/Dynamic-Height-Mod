...
public class StrollToPoi extends Behavior<PathfinderMob> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel serverLevel, PathfinderMob pathfinderMob) {
      ...
      return optional.isPresent()
         && serverLevel.dimension() == optional.get().dimension()
         && optional.get().pos().closerThan(pathfinderMob.position(), (double)this.maxDistanceFromPoi);
   }
   ...
   protected void start(ServerLevel serverLevel, PathfinderMob pathfinderMob, long l) {
      if (l > this.nextOkStartTime) {
         ...
         optional.ifPresent(
            globalPos -> brain.setMemory(MemoryModuleType.WALK_TARGET, new WalkTarget(globalPos.pos(), this.speedModifier, this.closeEnoughDist))
         );
