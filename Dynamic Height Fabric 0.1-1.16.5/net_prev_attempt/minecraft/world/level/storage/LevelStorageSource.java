...
public class LevelStorageSource {
    ...
    private static <T> Pair<WorldGenSettings, Lifecycle> readWorldGenSettings(Dynamic<T> dynamic, DataFixer dataFixer, int i) {
        ...
        return Pair.of((Object)dataResult.resultOrPartial(Util.prefix("WorldGenSettings: ", arg_0 -> ((Logger)LOGGER).error(arg_0))).orElseGet(() -> {
            Registry registry = (Registry)RegistryLookupCodec.create(Registry.DIMENSION_TYPE_REGISTRY).codec().parse(dynamic3).resultOrPartial(Util.prefix("Dimension type registry: ", arg_0 -> ((Logger)LOGGER).error(arg_0))).orElseThrow(() -> new IllegalStateException("Failed to get dimension registry"));
            Registry registry2 = (Registry)RegistryLookupCodec.create(Registry.BIOME_REGISTRY).codec().parse(dynamic3).resultOrPartial(Util.prefix("Biome registry: ", arg_0 -> ((Logger)LOGGER).error(arg_0))).orElseThrow(() -> new IllegalStateException("Failed to get biome registry"));
            Registry registry3 = (Registry)RegistryLookupCodec.create(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY).codec().parse(dynamic3).resultOrPartial(Util.prefix("Noise settings registry: ", arg_0 -> ((Logger)LOGGER).error(arg_0))).orElseThrow(() -> new IllegalStateException("Failed to get noise settings registry"));
    ...
    private static DataPackConfig readDataPackConfig(Dynamic<?> dynamic) {
        return DataPackConfig.CODEC.parse(dynamic).resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).orElse(DataPackConfig.DEFAULT);
    }
    ...
    private static DataPackConfig getDataPacks(File file, DataFixer dataFixer) {
        try {
            ...
            Dynamic dynamic = dataFixer.update(DataFixTypes.LEVEL.getType(), new Dynamic((DynamicOps)NbtOps.INSTANCE, (Object)compoundTag2), i, SharedConstants.getCurrentVersion().getWorldVersion());
    ...
    private static BiFunction<File, DataFixer, PrimaryLevelData> getLevelData(DynamicOps<Tag> dynamicOps, DataPackConfig dataPackConfig) {
        return (file, dataFixer) -> {
            try {
                ...
                compoundTag2.remove("Player");
                int i = compoundTag2.contains("DataVersion", 99) ? compoundTag2.getInt("DataVersion") : -1;
                Dynamic dynamic = dataFixer.update(DataFixTypes.LEVEL.getType(), new Dynamic(dynamicOps, (Object)compoundTag2), i, SharedConstants.getCurrentVersion().getWorldVersion());
                ...
                return PrimaryLevelData.parse((Dynamic<Tag>)dynamic, dataFixer, i, compoundTag3, levelSettings, levelVersion, (WorldGenSettings)pair.getFirst(), (Lifecycle)pair.getSecond());
            }
            catch (Exception exception) {
