...
public class PoiSection {
    ...
    public static Codec<PoiSection> codec(Runnable runnable) {
        return RecordCodecBuilder.create(instance -> instance.group((App)RecordCodecBuilder.point((Object)runnable), (App)Codec.BOOL.optionalFieldOf("Valid", (Object)false).forGetter(poiSection -> poiSection.isValid), (App)PoiRecord.codec(runnable).listOf().fieldOf("Records").forGetter(poiSection -> ImmutableList.copyOf((Collection)poiSection.records.values()))).apply((Applicative)instance, PoiSection::new)).orElseGet(Util.prefix("Failed to read POI section: ", arg_0 -> ((Logger)LOGGER).error(arg_0)), () -> new PoiSection(runnable, false, (List<PoiRecord>)ImmutableList.of()));
    }
