...
public class BeeDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void clearRemovedBees() {
      this.beeInfosPerEntity.entrySet().removeIf(entry -> this.minecraft.level.getEntity(entry.getValue().id) == null);
   }

   private void clearRemovedHives() {
      long l = this.minecraft.level.getGameTime() - 20L;
      this.hives.entrySet().removeIf(entry -> entry.getValue().lastSeen < l);
   }
   ...
   private void doRender() {
      ...
      this.hives.values().forEach(hiveInfo -> {
         if (blockPos.closerThan(hiveInfo.pos, 30.0)) {
            Set<UUID> set = map.get(hiveInfo.pos);
   ...
   private Map<BlockPos, Set<UUID>> createHiveBlacklistMap() {
      ...
      this.beeInfosPerEntity
         .values()
         .forEach(beeInfo -> beeInfo.blacklistedHives.forEach(blockPos -> map.computeIfAbsent(blockPos, blockPosx -> Sets.newHashSet()).add(beeInfo.getUuid())));
      return map;
   }
   ...
   private void renderFlowerInfos() {
      ...
      this.beeInfosPerEntity
         ...
         .forEach(beeInfo -> map.computeIfAbsent(beeInfo.flowerPos, blockPos -> Sets.newHashSet()).add(beeInfo.getUuid()));
      map.entrySet().forEach(entry -> {
         BlockPos blockPos = entry.getKey();
         Set<UUID> set = entry.getValue();
         Set<String> set2 = set.stream().map(DebugEntityNameGenerator::getEntityName).collect(Collectors.toSet());
         int i = 1;
   ...
   private static String getBeeUuidsAsString(Collection<UUID> collection) {
      if (collection.isEmpty()) {
         ...
         return collection.size() > 3
            ...
            : collection.stream().map(DebugEntityNameGenerator::getEntityName).collect(Collectors.toSet()).toString();
      }
   }
