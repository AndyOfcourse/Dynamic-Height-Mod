...
public class SignEditScreen extends Screen {
   ...
   public SignEditScreen(SignBlockEntity signBlockEntity) {
      ...
      this.messages = IntStream.range(0, 4).mapToObj(signBlockEntity::getMessage).map(Component::getString).toArray(i -> new String[i]);
   ...
   public void removed() {
      ...
      if (clientPacketListener != null) {
         clientPacketListener.send(
            new ServerboundSignUpdatePacket(this.sign.getBlockPos(), this.messages[0], this.messages[1], this.messages[2], this.messages[3])
         );
      }
