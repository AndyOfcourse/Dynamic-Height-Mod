...
public final class Shapes {
   ...
   public static double collide(Direction.Axis axis, AABB aABB, Stream<VoxelShape> stream, double d) {
      for(Iterator<VoxelShape> iterator = stream.iterator(); iterator.hasNext(); d = iterator.next().collide(axis, aABB, d)) {
   ...
   private static double collide(
      AABB aABB, LevelReader levelReader, double d, CollisionContext collisionContext, AxisCycle axisCycle, Stream<VoxelShape> stream
   ) {
                  ...
                  if (s < 3) {
                     ...
                     if ((s != 1 || blockState.hasLargeCollisionShape()) && (s != 2 || blockState.is(Blocks.MOVING_PISTON))) {
                        d = blockState.getCollisionShape(levelReader, mutableBlockPos, collisionContext)
                           .collide(
                              axis3, aABB.move((double)(-mutableBlockPos.getX()), (double)(-mutableBlockPos.getY()), (double)(-mutableBlockPos.getZ())), d
                           );
                        if (Math.abs(d) < 1.0E-7) {
                           return 0.0;
                        }
