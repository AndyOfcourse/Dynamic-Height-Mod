...
public class ChunkTaskPriorityQueue<T> {
    ...
    protected void resortChunkTasks(int i, ChunkPos chunkPos, int j) {
        ...
        if (list != null && !list.isEmpty()) {
            ((List)this.taskQueue.get(j).computeIfAbsent(chunkPos.toLong(), l -> Lists.newArrayList())).addAll(list);
            this.firstQueue = Math.min(this.firstQueue, j);
        }
    }

    protected void submit(Optional<T> optional, long l2, int i) {
        ((List)this.taskQueue.get(i).computeIfAbsent(l2, l -> Lists.newArrayList())).add(optional);
