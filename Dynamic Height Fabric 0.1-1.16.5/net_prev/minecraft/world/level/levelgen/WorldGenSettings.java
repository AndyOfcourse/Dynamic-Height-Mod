    ...
    public static WorldGenSettings create(RegistryAccess registryAccess, Properties properties) {
        ...
        switch (string5) {
            case "flat": {
                ...
                return new WorldGenSettings(l, bl, false, WorldGenSettings.withOverworld(registry, mappedRegistry, (ChunkGenerator)new FlatLevelSource(FlatLevelGeneratorSettings.CODEC.parse(dynamic).resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).orElseGet(() -> FlatLevelGeneratorSettings.getDefault(registry2)))));
            }
