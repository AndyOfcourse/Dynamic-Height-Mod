...
public final class Ingredient implements Predicate<ItemStack> {
   ...
   private void dissolve() {
      if (this.itemStacks == null) {
         this.itemStacks = Arrays.stream(this.values).flatMap(value -> value.getItems().stream()).distinct().toArray(i -> new ItemStack[i]);
      }
   }
   ...
   public static Ingredient fromNetwork(FriendlyByteBuf friendlyByteBuf) {
      ...
      return fromValues(Stream.<Ingredient.ItemValue>generate(() -> new Ingredient.ItemValue(friendlyByteBuf.readItem())).limit((long)i));
   }
