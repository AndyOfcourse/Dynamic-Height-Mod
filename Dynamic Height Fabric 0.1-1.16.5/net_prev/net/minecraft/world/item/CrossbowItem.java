      ...
      if (!level.isClientSide) {
         ...
         if (bl2) {
            projectile = new FireworkRocketEntity(
               level, itemStack2, livingEntity, livingEntity.getX(), livingEntity.getEyeY() - 0.15F, livingEntity.getZ(), true
            );
         } else {
         ...
         if (tooltipFlag.isAdvanced() && itemStack2.getItem() == Items.FIREWORK_ROCKET) {
            ...
            if (!list3.isEmpty()) {
               for(int i = 0; i < list3.size(); ++i) {
                  list3.set(i, new TextComponent("  ").append(list3.get(i)).withStyle(ChatFormatting.GRAY));
               }
