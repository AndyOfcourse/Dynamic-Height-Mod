...
public class HashCache {
   ...
   public void purgeStaleAndWrite() throws IOException {
      ...
      IOUtils.writeLines(
         this.newCache.entrySet().stream().map(entry -> (String)entry.getValue() + ' ' + this.path.relativize(entry.getKey())).collect(Collectors.toList()),
   ...
   private Stream<Path> walkOutputFiles() throws IOException {
      return Files.walk(this.path).filter(path -> !Objects.equals(this.cachePath, path) && !Files.isDirectory(path));
   }
}
