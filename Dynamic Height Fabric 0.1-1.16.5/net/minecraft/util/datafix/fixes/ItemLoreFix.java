...
public class ItemLoreFix extends DataFix {
                     ...
                     dynamic -> dynamic.update(
                           ...
                           dynamicx -> dynamicx.update(
                                 ...
                                 dynamicxx -> (Dynamic)DataFixUtils.orElse(
                                       dynamicxx.asStreamOpt().map(ItemLoreFix::fixLoreList).map(dynamicxx::createList).result(), dynamicxx
                                    )
                              )
                        )
                  )
   ...
   private static <T> Stream<Dynamic<T>> fixLoreList(Stream<Dynamic<T>> stream) {
      return stream.map(dynamic -> (Dynamic)DataFixUtils.orElse(dynamic.asString().map(ItemLoreFix::fixLoreEntry).map(dynamic::createString).result(), dynamic));
   }
