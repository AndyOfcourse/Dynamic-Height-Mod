...
public abstract class PropertyDispatch {
   ...
   private void verifyComplete() {
      ...
      List<Selector> list2 = stream.filter(selector -> !this.values.containsKey(selector)).collect(Collectors.toList());
      if (!list2.isEmpty()) {
         ...
         this.property1
            ...
            .forEach(
               comparable -> this.property2
                     .getPossibleValues()
                     .forEach(comparable2 -> this.select((T1)comparable, comparable2, biFunction.apply((T1)comparable, comparable2)))
            );
         return this;
      }
               ...
               comparable -> this.property2
                     ...
                     .forEach(
                        comparable2 -> this.property3
                              .getPossibleValues()
                              .forEach(
                                 comparable3 -> this.select(
                                       (T1)comparable, (T2)comparable2, comparable3, triFunction.apply((T1)comparable, (T2)comparable2, comparable3)
                                    )
                              )
                     )
            );
