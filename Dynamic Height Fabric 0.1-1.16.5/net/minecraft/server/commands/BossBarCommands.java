...
public class BossBarCommands {
   ...
   public static final SuggestionProvider<CommandSourceStack> SUGGEST_BOSS_BAR = (commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggestResource(
         ((CommandSourceStack)commandContext.getSource()).getServer().getCustomBossEvents().getIds(), suggestionsBuilder
      );
                        ...
                        Commands.literal("remove")
                           .then(
                              Commands.argument("id", ResourceLocationArgument.id())
                                 ...
                                 .executes(commandContext -> removeBar((CommandSourceStack)commandContext.getSource(), getBossBar(commandContext)))
                           )
                     ))
                  .then(Commands.literal("list").executes(commandContext -> listBars((CommandSourceStack)commandContext.getSource()))))
               .then(
                              ...
                              ((LiteralArgumentBuilder)Commands.literal("players")
                                    .executes(
                                       commandContext -> setPlayers(
                                             (CommandSourceStack)commandContext.getSource(), getBossBar(commandContext), Collections.emptyList()
                                          )
                                    ))
                                 .then(
                                    Commands.argument("targets", EntityArgument.players())
                                       .executes(
                                          commandContext -> setPlayers(
                                                (CommandSourceStack)commandContext.getSource(),
                                                getBossBar(commandContext),
                                                EntityArgument.getOptionalPlayers(commandContext, "targets")
                                             )
                                       )
                                 )
                           )
               ...
               Commands.literal("get")
                  .then(
                     ((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)Commands.argument("id", ResourceLocationArgument.id())
                                 ...
                                 .then(
                                    Commands.literal("value")
                                       .executes(commandContext -> getValue((CommandSourceStack)commandContext.getSource(), getBossBar(commandContext)))
                                 ))
                              .then(
                                 Commands.literal("max")
                                    .executes(commandContext -> getMax((CommandSourceStack)commandContext.getSource(), getBossBar(commandContext)))
                              ))
                           .then(
                              Commands.literal("visible")
                                 .executes(commandContext -> getVisible((CommandSourceStack)commandContext.getSource(), getBossBar(commandContext)))
                           ))
                        .then(
                           Commands.literal("players")
                              .executes(commandContext -> getPlayers((CommandSourceStack)commandContext.getSource(), getBossBar(commandContext)))
                        )
                  )
            )
      );
   ...
   private static int getValue(CommandSourceStack commandSourceStack, CustomBossEvent customBossEvent) {
      commandSourceStack.sendSuccess(
         new TranslatableComponent("commands.bossbar.get.value", customBossEvent.getDisplayName(), customBossEvent.getValue()), true
      );
