   ...
   public class OnlineServerEntry extends ServerSelectionList.Entry {
      ...
      public void render(PoseStack poseStack, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
         ...
         for(int p = 0; p < Math.min(list.size(), 2); ++p) {
            this.minecraft.font.draw(poseStack, list.get(p), (float)(k + 32 + 3), (float)(j + 12 + 9 * p), 8421504);
         }
      ...
      private void swap(int i, int j) {
         ...
         ServerSelectionList.Entry entry = this.screen.serverSelectionList.children().get(j);
