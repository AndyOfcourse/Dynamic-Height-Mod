...
public class FontSet implements AutoCloseable {
         ...
         for(GlyphProvider glyphProviderxx : list) {
            ...
            if (glyphInfo != null) {
               ...
               if (glyphInfo != MissingGlyph.INSTANCE) {
                  ((IntList)this.glyphsByWidth.computeIfAbsent(Mth.ceil(glyphInfo.getAdvance(false)), ix -> new IntArrayList())).add(i);
               }
               break;
            }
         }
   ...
   public GlyphInfo getGlyphInfo(int i) {
      return (GlyphInfo)this.glyphInfos.computeIfAbsent(i, ix -> (GlyphInfo)(ix == 32 ? SPACE_INFO : this.getRaw(ix)));
   }
   ...
   public BakedGlyph getGlyph(int i) {
      return (BakedGlyph)this.glyphs.computeIfAbsent(i, ix -> (BakedGlyph)(ix == 32 ? SPACE_GLYPH : this.stitch(this.getRaw(ix))));
   }
