...
public class LocateBiomeCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
      commandDispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("locatebiome").requires(commandSourceStack -> commandSourceStack.hasPermission(2)))
            .then(
               Commands.argument("biome", ResourceLocationArgument.id())
                  ...
                  .executes(
                     commandContext -> locateBiome(
                           (CommandSourceStack)commandContext.getSource(), (ResourceLocation)commandContext.getArgument("biome", ResourceLocation.class)
                        )
                  )
            )
      );
   ...
   private static int locateBiome(CommandSourceStack commandSourceStack, ResourceLocation resourceLocation) throws CommandSyntaxException {
      Biome biome = commandSourceStack.getServer()
         .registryAccess()
