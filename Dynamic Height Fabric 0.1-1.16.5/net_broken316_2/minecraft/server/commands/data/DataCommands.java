...
public class DataCommands {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
      LiteralArgumentBuilder<CommandSourceStack> literalArgumentBuilder = (LiteralArgumentBuilder)Commands.literal("data")
                  ...
                  (argumentBuilder, dataManipulatorDecorator) -> argumentBuilder.then(
                           Commands.literal("insert")
                              .then(
                                 Commands.argument("index", IntegerArgumentType.integer())
                                    .then(dataManipulatorDecorator.create((commandContext, compoundTag, nbtPath, list) -> {
                        ...
                        .then(
                           Commands.literal("prepend")
                              .then(
                                 dataManipulatorDecorator.create((commandContext, compoundTag, nbtPath, list) -> insertAtIndex(0, compoundTag, nbtPath, list))
                              )
                        )
                        .then(
                           Commands.literal("append")
                              .then(
                                 dataManipulatorDecorator.create((commandContext, compoundTag, nbtPath, list) -> insertAtIndex(-1, compoundTag, nbtPath, list))
                              )
                        )
                        .then(
                           Commands.literal("set")
                              .then(
                                 dataManipulatorDecorator.create(
                                    (commandContext, compoundTag, nbtPath, list) -> nbtPath.set(compoundTag, ((Tag)Iterables.getLast(list))::copy)
                                 )
                              )
                        )
         ...
         dataProvider.wrap(
            ...
            argumentBuilder -> {
               ...
               for(DataCommands.DataProvider dataProvider2 : SOURCE_PROVIDERS) {
                  biConsumer.accept(
                     argumentBuilder2,
                     dataManipulator -> dataProvider2.wrap(Commands.literal("from"), argumentBuilderx -> argumentBuilderx.executes(commandContext -> {
