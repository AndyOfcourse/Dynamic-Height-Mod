...
public class GossipContainer {
   ...
   private Collection<GossipContainer.GossipEntry> selectGossipsForTransfer(Random random, int i) {
      ...
      if (list.isEmpty()) {
         ...
         for(int k = 0; k < list.size(); ++k) {
            GossipContainer.GossipEntry gossipEntry = list.get(k);
   ...
   private GossipContainer.EntityGossips getOrCreate(UUID uUID) {
      return this.gossips.computeIfAbsent(uUID, uUIDx -> new GossipContainer.EntityGossips());
   }
         ...
         return new Dynamic(
            ...
            dynamicOps.createMap(
               ImmutableMap.of(
                  ...
                  SerializableUUID.CODEC.encodeStart(dynamicOps, this.target).result().orElseThrow(RuntimeException::new),
