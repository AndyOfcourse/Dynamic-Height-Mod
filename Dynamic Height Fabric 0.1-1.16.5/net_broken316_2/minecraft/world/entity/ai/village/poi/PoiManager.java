...
public class PoiManager extends SectionStorage<PoiSection> {
   ...
   public Stream<PoiRecord> getInChunk(Predicate<PoiType> predicate, ChunkPos chunkPos, PoiManager.Occupancy occupancy) {
      return IntStream.range(0, 16)
         ...
         .flatMap(optional -> optional.get().getRecords(predicate, occupancy));
   }
   ...
   public Optional<BlockPos> getRandom(
      ...
      List<PoiRecord> list = this.getInRange(predicate, blockPos, i, occupancy).collect(Collectors.toList());
   ...
   private boolean isVillageCenter(long l) {
      ...
      return optional == null
         ? false
         : optional.<Boolean>map(poiSection -> poiSection.getRecords(PoiType.ALL, PoiManager.Occupancy.IS_OCCUPIED).count() > 0L).orElse(false);
   }
