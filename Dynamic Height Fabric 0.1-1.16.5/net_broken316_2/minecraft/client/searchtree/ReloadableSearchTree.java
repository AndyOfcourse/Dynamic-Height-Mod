   ...
   public List<T> search(String string) {
      ...
      if (i < 0) {
         ...
         return Lists.newArrayList(
            new ReloadableIdSearchTree.IntersectionIterator<T>(
               list.iterator(),
               new ReloadableSearchTree.MergingUniqueIterator<T>(list2.iterator(), list3.iterator(), this::comparePosition),
   ...
   static class MergingUniqueIterator<T> extends AbstractIterator<T> {
      ...
      protected T computeNext() {
         ...
         if (bl && bl2) {
            ...
            int i = this.orderT.compare((T)this.firstIterator.peek(), (T)this.secondIterator.peek());
            if (i == 0) {
