...
public class ReplaceItemCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
      commandDispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("replaceitem")
                                             ...
                                             .executes(
                                                commandContext -> setBlockItem(
                                                      ...
                                                      ItemArgument.getItem(commandContext, "item")
                                                         .createItemStack(IntegerArgumentType.getInteger(commandContext, "count"), true)
                                                   )
                                             )
                                       )
                                 )
      ...
      if (!(blockEntity instanceof Container)) {
         ...
         if (i >= 0 && i < container.getContainerSize()) {
            ...
            commandSourceStack.sendSuccess(
               new TranslatableComponent("commands.replaceitem.block.success", blockPos.getX(), blockPos.getY(), blockPos.getZ(), itemStack.getDisplayName()),
               true
            );
            return 1;
         } else {
   ...
   private static int setEntityItem(CommandSourceStack commandSourceStack, Collection<? extends Entity> collection, int i, ItemStack itemStack) throws CommandSyntaxException {
      ...
      if (list.isEmpty()) {
         ...
         if (list.size() == 1) {
            commandSourceStack.sendSuccess(
               new TranslatableComponent("commands.replaceitem.entity.success.single", list.iterator().next().getDisplayName(), itemStack.getDisplayName()),
               true
            );
         } else {
            commandSourceStack.sendSuccess(
               new TranslatableComponent("commands.replaceitem.entity.success.multiple", list.size(), itemStack.getDisplayName()), true
            );
         }
