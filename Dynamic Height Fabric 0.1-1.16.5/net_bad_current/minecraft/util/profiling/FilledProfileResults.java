...
public class FilledProfileResults implements ProfileResults {
            ...
            (string, profilerPathEntry) -> {
               ...
               if (!object2LongMap.isEmpty()) {
                  ...
                  object2LongMap.forEach(
                     (stringx, long_) -> map.computeIfAbsent(stringx, stringxx -> new FilledProfileResults.CounterCollector()).addValue(list.iterator(), long_)
                  );
               }
            }
         );
   ...
   private void appendProfilerResults(int i, String string, StringBuilder stringBuilder) {
      ...
      Object2LongMap<String> object2LongMap = ((ProfilerPathEntry)ObjectUtils.firstNonNull(new ProfilerPathEntry[]{this.entries.get(string), EMPTY}))
         .getCounters();
      object2LongMap.forEach(
   ...
   private void appendCounterResults(int i, String string, FilledProfileResults.CounterCollector counterCollector, int j, StringBuilder stringBuilder) {
      ...
      counterCollector.children
         ...
         .forEach(entry -> this.appendCounterResults(i + 1, entry.getKey(), entry.getValue(), j, stringBuilder));
   }
   ...
   static class CounterCollector {
      ...
      public void addValue(Iterator<String> iterator, long l) {
         ...
         if (!iterator.hasNext()) {
            ...
            this.children.computeIfAbsent(iterator.next(), string -> new FilledProfileResults.CounterCollector()).addValue(iterator, l);
         }
      }
   }
}
