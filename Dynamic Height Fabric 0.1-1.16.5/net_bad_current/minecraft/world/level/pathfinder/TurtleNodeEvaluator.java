...
public class TurtleNodeEvaluator extends WalkNodeEvaluator {
   ...
   private Node getAcceptedNode(int i, int j, int k, int l, double d) {
      ...
      if (e - d > 1.125) {
         ...
         BlockPathTypes blockPathTypes = this.getBlockPathType(
            this.level, i, j, k, this.mob, this.entityWidth, this.entityHeight, this.entityDepth, false, false
         );
         ...
         if (blockPathTypes != BlockPathTypes.WATER && blockPathTypes != BlockPathTypes.WALKABLE) {
            ...
            if (blockPathTypes == BlockPathTypes.OPEN) {
               AABB aABB = new AABB(
                  ...
                  (double)k - g + 0.5,
                  (double)i + g + 0.5,
                  (double)((float)j + this.mob.getBbHeight()),
                  (double)k + g + 0.5
               );
