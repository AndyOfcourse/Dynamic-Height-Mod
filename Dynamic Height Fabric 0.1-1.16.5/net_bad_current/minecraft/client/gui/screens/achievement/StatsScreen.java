...
public class StatsScreen extends Screen implements StatsUpdateListener {
   ...
   public void initButtons() {
      this.addButton(
         new Button(
            this.width / 2 - 120, this.height - 52, 80, 20, new TranslatableComponent("stat.generalButton"), buttonx -> this.setActiveList(this.statsList)
         )
      );
      Button button = this.addButton(
         new Button(
            this.width / 2 - 40, this.height - 52, 80, 20, new TranslatableComponent("stat.itemsButton"), buttonx -> this.setActiveList(this.itemStatsList)
         )
      );
      Button button2 = this.addButton(
         new Button(
            this.width / 2 + 40, this.height - 52, 80, 20, new TranslatableComponent("stat.mobsButton"), buttonx -> this.setActiveList(this.mobsStatsList)
         )
      );
      this.addButton(
         ...
         public void render(PoseStack poseStack, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
            ...
            for(int p = 0; p < StatsScreen.this.itemStatsList.blockColumns.size(); ++p) {
               ...
               if (item instanceof BlockItem) {
                  stat = StatsScreen.this.itemStatsList.blockColumns.get(p).get(((BlockItem)item).getBlock());
               } else {
                  stat = null;
               }
            ...
            for(int p = 0; p < StatsScreen.this.itemStatsList.itemColumns.size(); ++p) {
               this.renderStat(
                  poseStack,
                  StatsScreen.this.itemStatsList.itemColumns.get(p).get(item),
