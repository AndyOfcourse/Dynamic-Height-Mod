...
public class GameRules {
   ...
   public GameRules copy() {
      return new GameRules(
         this.rules.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, entry -> ((GameRules.Value)entry.getValue()).copy()))
      );
   }
   ...
   public static class BooleanValue extends GameRules.Value<GameRules.BooleanValue> {
      ...
      private static GameRules.Type<GameRules.BooleanValue> create(boolean bl, BiConsumer<MinecraftServer, GameRules.BooleanValue> biConsumer) {
         return new GameRules.Type<>(
            BoolArgumentType::bool, type -> new GameRules.BooleanValue(type, bl), biConsumer, GameRules.GameRuleTypeVisitor::visitBoolean
         );
      }
