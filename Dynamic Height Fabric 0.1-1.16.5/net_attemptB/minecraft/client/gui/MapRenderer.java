...
public class MapRenderer implements AutoCloseable {
   ...
   private MapRenderer.MapInstance getMapInstance(MapItemSavedData mapItemSavedData) {
      MapRenderer.MapInstance mapInstance = this.maps.get(mapItemSavedData.getId());
      if (mapInstance == null) {
      ...
      private void draw(PoseStack poseStack, MultiBufferSource multiBufferSource, boolean bl, int i) {
         ...
         for(MapDecoration mapDecoration : this.data.decorations.values()) {
            if (!bl || mapDecoration.renderOnFrame()) {
               poseStack.pushPose();
               poseStack.translate(
                  (double)(0.0F + (float)mapDecoration.getX() / 2.0F + 64.0F), (double)(0.0F + (float)mapDecoration.getY() / 2.0F + 64.0F), -0.02F
               );
               ...
               if (mapDecoration.getName() != null) {
                  ...
                  poseStack.pushPose();
                  poseStack.translate(
                     (double)(0.0F + (float)mapDecoration.getX() / 2.0F + 64.0F - p * q / 2.0F),
                     (double)(0.0F + (float)mapDecoration.getY() / 2.0F + 64.0F + 4.0F),
                     -0.025F
                  );
