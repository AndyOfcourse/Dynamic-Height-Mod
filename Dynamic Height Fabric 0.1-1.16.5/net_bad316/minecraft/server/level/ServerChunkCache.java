...
public class ServerChunkCache extends ChunkSource {
   ...
   public ChunkAccess getChunk(int i, int j, ChunkStatus chunkStatus, boolean bl) {
      if (Thread.currentThread() != this.mainThread) {
         return CompletableFuture.<ChunkAccess>supplyAsync(() -> this.getChunk(i, j, chunkStatus, bl), this.mainThreadProcessor).join();
      } else {
         ...
         ChunkAccess chunkAccess = (ChunkAccess)((Either)completableFuture.join()).map(chunkAccessx -> chunkAccessx, chunkLoadingFailure -> {
            if (bl) {
   ...
   public CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> getChunkFuture(int i, int j, ChunkStatus chunkStatus, boolean bl) {
      ...
      if (bl2) {
         ...
         completableFuture = CompletableFuture.<CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>>supplyAsync(
               () -> this.getChunkFutureMainThread(i, j, chunkStatus, bl), this.mainThreadProcessor
            )
   ...
   private boolean checkChunkFuture(long l, Function<ChunkHolder, CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>>> function) {
      ...
      if (chunkHolder == null) {
         ...
         Either<LevelChunk, ChunkHolder.ChunkLoadingFailure> either = (Either)function.apply(chunkHolder).getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK);
