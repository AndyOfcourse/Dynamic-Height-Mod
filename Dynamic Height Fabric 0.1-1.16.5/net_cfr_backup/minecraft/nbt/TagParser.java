...
public class TagParser {
    ...
    public static final Dynamic2CommandExceptionType ERROR_INSERT_MIXED_LIST = new Dynamic2CommandExceptionType((object, object2) -> new TranslatableComponent("argument.nbt.list.mixed", new Object[]{object, object2}));
    public static final Dynamic2CommandExceptionType ERROR_INSERT_MIXED_ARRAY = new Dynamic2CommandExceptionType((object, object2) -> new TranslatableComponent("argument.nbt.array.mixed", new Object[]{object, object2}));
    public static final DynamicCommandExceptionType ERROR_INVALID_ARRAY = new DynamicCommandExceptionType(object -> new TranslatableComponent("argument.nbt.array.invalid", new Object[]{object}));
    ...
    protected Tag readTypedValue() throws CommandSyntaxException {
        ...
        if (StringReader.isQuotedStringStart((char)this.reader.peek())) {
            return StringTag.valueOf((String)this.reader.readQuotedString());
        }
    ...
    private Tag type(String string) {
        try {
            if (FLOAT_PATTERN.matcher(string).matches()) {
                return FloatTag.valueOf((float)Float.parseFloat(string.substring(0, string.length() - 1)));
            }
            if (BYTE_PATTERN.matcher(string).matches()) {
                return ByteTag.valueOf((byte)Byte.parseByte(string.substring(0, string.length() - 1)));
            }
            if (LONG_PATTERN.matcher(string).matches()) {
                return LongTag.valueOf((long)Long.parseLong(string.substring(0, string.length() - 1)));
            }
            if (SHORT_PATTERN.matcher(string).matches()) {
                return ShortTag.valueOf((short)Short.parseShort(string.substring(0, string.length() - 1)));
            }
            if (INT_PATTERN.matcher(string).matches()) {
                return IntTag.valueOf((int)Integer.parseInt(string));
            }
            if (DOUBLE_PATTERN.matcher(string).matches()) {
                return DoubleTag.valueOf((double)Double.parseDouble(string.substring(0, string.length() - 1)));
            }
            if (DOUBLE_PATTERN_NOSUFFIX.matcher(string).matches()) {
                return DoubleTag.valueOf((double)Double.parseDouble(string));
            }
