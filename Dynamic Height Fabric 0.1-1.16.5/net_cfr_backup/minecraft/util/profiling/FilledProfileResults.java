...
implements ProfileResults {
    ...
    private static final ProfilerPathEntry EMPTY = new /* Unavailable Anonymous Inner Class!! */;
    private static final Splitter SPLITTER = Splitter.on((char)'\u001e');
    private static final Comparator<Map.Entry<String, CounterCollector>> COUNTER_ENTRY_COMPARATOR = Map.Entry.comparingByValue(Comparator.comparingLong(counterCollector -> CounterCollector.method_24265((CounterCollector)counterCollector))).reversed();
    ...
    private Map<String, CounterCollector> getCounterValues() {
        ...
        this.entries.forEach((string, profilerPathEntry) -> {
            Object2LongMap object2LongMap = profilerPathEntry.getCounters();
            if (!object2LongMap.isEmpty()) {
                List list = SPLITTER.splitToList((CharSequence)string);
                object2LongMap.forEach((string2, long_) -> map.computeIfAbsent(string2, string -> new CounterCollector(null)).addValue(list.iterator(), long_.longValue()));
            }
        });
        return map;
    }
    ...
    private void appendProfilerResults(int i, String string2, StringBuilder stringBuilder) {
        ...
        Object2LongMap object2LongMap = ((ProfilerPathEntry)ObjectUtils.firstNonNull((Object[])new ProfilerPathEntry[]{this.entries.get(string2), EMPTY})).getCounters();
    ...
    private void appendCounterResults(int i, String string, CounterCollector counterCollector, int j, StringBuilder stringBuilder) {
        FilledProfileResults.indentLine(stringBuilder, i).append(string).append(" total:").append(CounterCollector.method_24268((CounterCollector)counterCollector)).append('/').append(CounterCollector.method_24265((CounterCollector)counterCollector)).append(" average: ").append(CounterCollector.method_24268((CounterCollector)counterCollector) / (long)j).append('/').append(CounterCollector.method_24265((CounterCollector)counterCollector) / (long)j).append('\n');
        CounterCollector.method_24269((CounterCollector)counterCollector).entrySet().stream().sorted(COUNTER_ENTRY_COMPARATOR).forEach(entry -> this.appendCounterResults(i + 1, (String)entry.getKey(), (CounterCollector)entry.getValue(), j, stringBuilder));
    }
    ...
    private void appendCounters(Map<String, CounterCollector> map, StringBuilder stringBuilder, int i) {
        map.forEach((string, counterCollector) -> {
            ...
            this.appendCounterResults(0, "root", (CounterCollector)CounterCollector.method_24269((CounterCollector)counterCollector).get("root"), i, stringBuilder);
            stringBuilder.append("\n\n");
        });
    }
