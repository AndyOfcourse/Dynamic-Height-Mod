...
extends AbstractUUIDFix {
    ...
    public TypeRewriteRule makeRule() {
        OpticFinder opticFinder = DSL.fieldFinder((String)"id", (Type)DSL.named((String)References.ITEM_NAME.typeName(), (Type)NamespacedSchema.namespacedString()));
    ...
    private Dynamic<?> updateAttributeModifiers(Dynamic<?> dynamic) {
        return dynamic.update("AttributeModifiers", dynamic22 -> dynamic.createList(dynamic22.asStream().map(dynamic -> ItemStackUUIDFix.replaceUUIDLeastMost((Dynamic)dynamic, (String)"UUID", (String)"UUID").orElse(dynamic))));
    }

    private Dynamic<?> updateSkullOwner(Dynamic<?> dynamic2) {
        return dynamic2.update("SkullOwner", dynamic -> ItemStackUUIDFix.replaceUUIDString((Dynamic)dynamic, (String)"Id", (String)"Id").orElse(dynamic));
    }
}
