...
public class PlayerAdvancements {
    ...
    private static final TypeToken<Map<ResourceLocation, AdvancementProgress>> TYPE_TOKEN = new /* Unavailable Anonymous Inner Class!! */;
        ...
        if (advancementProgress.grantProgress(string)) {
            ...
            if (!bl2 && advancementProgress.isDone()) {
                ...
                if (advancement.getDisplay() != null && advancement.getDisplay().shouldAnnounceChat() && this.player.level.getGameRules().getBoolean(GameRules.RULE_ANNOUNCE_ADVANCEMENTS)) {
                    this.playerList.broadcastMessage((Component)new TranslatableComponent("chat.type.advancement." + advancement.getDisplay().getFrame().getName(), new Object[]{this.player.getDisplayName(), advancement.getChatComponent()}), ChatType.SYSTEM, Util.NIL_UUID);
                }
            }
        }
    ...
    private void registerListeners(Advancement advancement) {
        ...
        for (Map.Entry entry : advancement.getCriteria().entrySet()) {
            ...
            if (criterionProgress == null || criterionProgress.isDone() || (criterionTriggerInstance = ((Criterion)entry.getValue()).getTrigger()) == null || (criterionTrigger = CriteriaTriggers.getCriterion((ResourceLocation)criterionTriggerInstance.getCriterion())) == null) continue;
    ...
    private void unregisterListeners(Advancement advancement) {
        ...
        for (Map.Entry entry : advancement.getCriteria().entrySet()) {
            ...
            CriterionTriggerInstance criterionTriggerInstance;
            CriterionProgress criterionProgress = advancementProgress.getCriterion((String)entry.getKey());
            if (criterionProgress == null || !criterionProgress.isDone() && !advancementProgress.isDone() || (criterionTriggerInstance = ((Criterion)entry.getValue()).getTrigger()) == null || (criterionTrigger = CriteriaTriggers.getCriterion((ResourceLocation)criterionTriggerInstance.getCriterion())) == null) continue;
    ...
    public void flushDirty(ServerPlayer serverPlayer) {
        if (this.isFirstPacket || !this.visibilityChanged.isEmpty() || !this.progressChanged.isEmpty()) {
            ...
            if (this.isFirstPacket || !map.isEmpty() || !set.isEmpty() || !set2.isEmpty()) {
                serverPlayer.connection.send((Packet)new ClientboundUpdateAdvancementsPacket(this.isFirstPacket, (Collection)set, (Set)set2, (Map)map));
                this.visibilityChanged.clear();
                this.progressChanged.clear();
            }
        }
    ...
    public void setSelectedTab(@Nullable Advancement advancement) {
        ...
        if (advancement2 != this.lastSelectedTab) {
            this.player.connection.send((Packet)new ClientboundSelectAdvancementsTabPacket(this.lastSelectedTab == null ? null : this.lastSelectedTab.getId()));
        }
    }
