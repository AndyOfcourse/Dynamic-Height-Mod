...
public class GameProfileCache {
    ...
    private static GameProfile lookupGameProfile(GameProfileRepository gameProfileRepository, String string) {
        ...
        1 profileLookupCallback = new /* Unavailable Anonymous Inner Class!! */;
        gameProfileRepository.findProfilesByNames(new String[]{string}, Agent.MINECRAFT, (ProfileLookupCallback)profileLookupCallback);
        GameProfile gameProfile = (GameProfile)atomicReference.get();
        if (!GameProfileCache.usesAuthentication() && gameProfile == null) {
            UUID uUID = Player.createPlayerUUID((GameProfile)new GameProfile(null, string));
    ...
    public void add(GameProfile gameProfile) {
        ...
        GameProfileInfo gameProfileInfo = new GameProfileInfo(gameProfile, date, null);
    ...
    public GameProfile get(String string) {
        ...
        boolean bl = false;
        if (gameProfileInfo != null && new Date().getTime() >= GameProfileInfo.method_14521((GameProfileInfo)gameProfileInfo).getTime()) {
    ...
    public List<GameProfileInfo> load() {
        ...
        try (BufferedReader reader2 = Files.newReader((File)this.file, (Charset)StandardCharsets.UTF_8);){
    ...
    private static GameProfileInfo readGameProfile(JsonElement jsonElement, DateFormat dateFormat) {
        if (jsonElement.isJsonObject()) {
            ...
            return new GameProfileInfo(new GameProfile(uUID, string2), date, null);
        }
        return null;
    }
}
