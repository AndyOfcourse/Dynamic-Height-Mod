...
implements AutoCloseable {
    ...
    public void reload(List<GlyphProvider> list) {
        ...
        this.missingGlyph = this.stitch((RawGlyph)MissingGlyph.INSTANCE);
        this.whiteGlyph = this.stitch((RawGlyph)WhiteGlyph.INSTANCE);
        ...
        intSet.forEach(i2 -> {
            for (GlyphProvider glyphProvider : list) {
                Object glyphInfo = i2 == 32 ? SPACE_INFO : glyphProvider.getGlyph(i2);
                if (glyphInfo == null) continue;
                set.add(glyphProvider);
                if (glyphInfo == MissingGlyph.INSTANCE) break;
                ((IntList)this.glyphsByWidth.computeIfAbsent(Mth.ceil((float)glyphInfo.getAdvance(false)), i -> new IntArrayList())).add(i2);
                break;
            }
        });
    ...
    public GlyphInfo getGlyphInfo(int i2) {
        return (GlyphInfo)this.glyphInfos.computeIfAbsent(i2, i -> i == 32 ? SPACE_INFO : this.getRaw(i));
    }
    ...
    public BakedGlyph getGlyph(int i2) {
        return (BakedGlyph)this.glyphs.computeIfAbsent(i2, i -> i == 32 ? SPACE_GLYPH : this.stitch(this.getRaw(i)));
    }
    ...
    private BakedGlyph stitch(RawGlyph rawGlyph) {
        ...
        this.textureManager.register(fontTexture2.getName(), (AbstractTexture)fontTexture2);
    ...
    public BakedGlyph getRandomGlyph(GlyphInfo glyphInfo) {
        IntList intList = (IntList)this.glyphsByWidth.get(Mth.ceil((float)glyphInfo.getAdvance(false)));
