...
IdMap<T> {
    ...
    private static <T> ResourceKey<Registry<T>> createRegistryKey(String string) {
        return ResourceKey.createRegistryKey((ResourceLocation)new ResourceLocation(string));
    }
    ...
    public static <T extends WritableRegistry<?>> void checkRegistry(WritableRegistry<T> writableRegistry) {
        writableRegistry.forEach(writableRegistry2 -> {
            ...
            if (writableRegistry2 instanceof DefaultedRegistry) {
                ...
                Validate.notNull((Object)writableRegistry2.get(resourceLocation), (String)("Missing default of DefaultedMappedRegistry: " + resourceLocation), (Object[])new Object[0]);
            }
        });
    }
    ...
    private static <T, R extends WritableRegistry<T>> R internalRegister(ResourceKey<? extends Registry<T>> resourceKey, R writableRegistry, Supplier<T> supplier, Lifecycle lifecycle) {
        ...
        return (R)((WritableRegistry)writableRegistry2.register(resourceKey, writableRegistry, lifecycle));
    }
    ...
    public <U> DataResult<Pair<T, U>> decode(DynamicOps<U> dynamicOps, U object2) {
        ...
        return ResourceLocation.CODEC.decode(dynamicOps, object2).flatMap(pair -> {
            T object = this.get((ResourceLocation)pair.getFirst());
            if (object == null) {
                return DataResult.error((String)("Unknown registry key: " + pair.getFirst()));
            }
            return DataResult.success((Object)Pair.of(object, (Object)pair.getSecond()), (Lifecycle)this.lifecycle(object));
        });
    }
    ...
    public static <V, T extends V> T register(Registry<V> registry, ResourceLocation resourceLocation, T object) {
        return (T)((WritableRegistry)registry).register(ResourceKey.create(registry.key, (ResourceLocation)resourceLocation), object, Lifecycle.stable());
    }

    public static <V, T extends V> T registerMapping(Registry<V> registry, int i, String string, T object) {
        return (T)((WritableRegistry)registry).registerMapping(i, ResourceKey.create(registry.key, (ResourceLocation)new ResourceLocation(string)), object, Lifecycle.stable());
    }
