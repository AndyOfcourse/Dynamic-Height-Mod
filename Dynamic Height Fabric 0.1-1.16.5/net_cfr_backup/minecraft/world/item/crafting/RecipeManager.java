...
extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> map, ResourceManager resourceManager, ProfilerFiller profilerFiller) {
        ...
        for (Map.Entry<ResourceLocation, JsonElement> entry2 : map.entrySet()) {
            ...
            try {
                Recipe<?> recipe = RecipeManager.fromJson(resourceLocation, GsonHelper.convertToJsonObject((JsonElement)entry2.getValue(), (String)"top element"));
                map2.computeIfAbsent(recipe.getType(), recipeType -> ImmutableMap.builder()).put((Object)resourceLocation, recipe);
            }
    ...
    public <C extends Container, T extends Recipe<C>> Optional<T> getRecipeFor(RecipeType<T> recipeType, C container, Level level) {
        return this.byType(recipeType).values().stream().flatMap(recipe -> Util.toStream((Optional)recipeType.tryMatch(recipe, level, container))).findFirst();
    }
    ...
    public <C extends Container, T extends Recipe<C>> List<T> getRecipesFor(RecipeType<T> recipeType, C container, Level level) {
        return this.byType(recipeType).values().stream().flatMap(recipe -> Util.toStream((Optional)recipeType.tryMatch(recipe, level, container))).sorted(Comparator.comparing(recipe -> recipe.getResultItem().getDescriptionId())).collect(Collectors.toList());
    }
    ...
    public <C extends Container, T extends Recipe<C>> NonNullList<ItemStack> getRemainingItemsFor(RecipeType<T> recipeType, C container, Level level) {
        ...
        NonNullList nonNullList = NonNullList.withSize((int)container.getContainerSize(), (Object)ItemStack.EMPTY);
        for (int i = 0; i < nonNullList.size(); ++i) {
            nonNullList.set(i, (Object)container.getItem(i));
        }
        return nonNullList;
    }
    ...
    public static Recipe<?> fromJson(ResourceLocation resourceLocation, JsonObject jsonObject) {
        String string = GsonHelper.getAsString((JsonObject)jsonObject, (String)"type");
        return ((RecipeSerializer)Registry.RECIPE_SERIALIZER.getOptional(new ResourceLocation(string)).orElseThrow(() -> new JsonSyntaxException("Invalid or unsupported recipe type '" + string + "'"))).fromJson(resourceLocation, jsonObject);
    }
