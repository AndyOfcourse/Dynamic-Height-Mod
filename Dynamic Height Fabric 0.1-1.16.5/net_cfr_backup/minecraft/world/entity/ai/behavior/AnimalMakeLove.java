...
extends Behavior<Animal> {
    ...
    public AnimalMakeLove(EntityType<? extends Animal> entityType, float f) {
        super((Map)ImmutableMap.of((Object)MemoryModuleType.VISIBLE_LIVING_ENTITIES, (Object)MemoryStatus.VALUE_PRESENT, (Object)MemoryModuleType.BREED_TARGET, (Object)MemoryStatus.VALUE_ABSENT, (Object)MemoryModuleType.WALK_TARGET, (Object)MemoryStatus.REGISTERED, (Object)MemoryModuleType.LOOK_TARGET, (Object)MemoryStatus.REGISTERED), 325);
    ...
    protected void start(ServerLevel serverLevel, Animal animal, long l) {
        ...
        animal.getBrain().setMemory(MemoryModuleType.BREED_TARGET, (Object)animal2);
        animal2.getBrain().setMemory(MemoryModuleType.BREED_TARGET, (Object)animal);
        BehaviorUtils.lockGazeAndWalkToEachOther((LivingEntity)animal, (LivingEntity)animal2, (float)this.speedModifier);
    ...
    protected boolean canStillUse(ServerLevel serverLevel, Animal animal, long l) {
        ...
        return animal2.isAlive() && animal.canMate(animal2) && BehaviorUtils.entityIsVisible((Brain)animal.getBrain(), (LivingEntity)animal2) && l <= this.spawnChildAtTime;
    }

    protected void tick(ServerLevel serverLevel, Animal animal, long l) {
        Animal animal2 = this.getBreedTarget(animal);
        BehaviorUtils.lockGazeAndWalkToEachOther((LivingEntity)animal, (LivingEntity)animal2, (float)this.speedModifier);
        if (!animal.closerThan((Entity)animal2, 3.0)) {
            return;
        }
    ...
    private Optional<? extends Animal> findValidBreedPartner(Animal animal) {
        return ((List)animal.getBrain().getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES).get()).stream().filter(livingEntity -> livingEntity.getType() == this.partnerType).map(livingEntity -> (Animal)livingEntity).filter(arg_0 -> ((Animal)animal).canMate(arg_0)).findFirst();
    }
