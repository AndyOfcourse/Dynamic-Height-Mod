...
public class PoiSection {
    ...
    public static Codec<PoiSection> codec(Runnable runnable) {
        return RecordCodecBuilder.create(instance -> instance.group((App)RecordCodecBuilder.point((Object)runnable), (App)Codec.BOOL.optionalFieldOf("Valid", (Object)false).forGetter(poiSection -> poiSection.isValid), (App)PoiRecord.codec((Runnable)runnable).listOf().fieldOf("Records").forGetter(poiSection -> ImmutableList.copyOf((Collection)poiSection.records.values()))).apply((Applicative)instance, PoiSection::new)).orElseGet(Util.prefix((String)"Failed to read POI section: ", arg_0 -> ((Logger)LOGGER).error(arg_0)), () -> new PoiSection(runnable, false, (List<PoiRecord>)ImmutableList.of()));
    }
    ...
    private boolean add(PoiRecord poiRecord) {
        ...
        short s = SectionPos.sectionRelativePos((BlockPos)blockPos);
        ...
        if (poiRecord2 != null) {
            ...
            if (SharedConstants.IS_RUNNING_IN_IDE) {
                throw (IllegalStateException)Util.pauseInIde((Throwable)new IllegalStateException(string));
            }
            LOGGER.error(string);
        }
    ...
    public void remove(BlockPos blockPos) {
        PoiRecord poiRecord = (PoiRecord)this.records.remove(SectionPos.sectionRelativePos((BlockPos)blockPos));
        if (poiRecord == null) {
        ...
        supplierArray[0] = () -> ((PoiRecord)poiRecord).getPoiType();
        supplierArray[1] = () -> ((PoiRecord)poiRecord).getPos();
        LOGGER.debug("Removed POI of type {} @ {}", supplierArray);
        this.setDirty.run();
    }

    public boolean release(BlockPos blockPos) {
        PoiRecord poiRecord = (PoiRecord)this.records.get(SectionPos.sectionRelativePos((BlockPos)blockPos));
        if (poiRecord == null) {
            throw (IllegalStateException)Util.pauseInIde((Throwable)new IllegalStateException("POI never registered at " + blockPos));
        }
    ...
    private /* synthetic */ void method_20352(Short2ObjectMap short2ObjectMap, BlockPos blockPos, PoiType poiType) {
        short s = SectionPos.sectionRelativePos((BlockPos)blockPos);
