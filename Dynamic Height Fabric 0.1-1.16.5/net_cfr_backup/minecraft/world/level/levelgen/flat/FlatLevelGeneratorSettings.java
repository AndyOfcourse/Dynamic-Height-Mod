...
public class FlatLevelGeneratorSettings {
    ...
    public FlatLevelGeneratorSettings(Registry<Biome> registry, StructureSettings structureSettings, List<FlatLayerInfo> list, boolean bl, boolean bl2, Optional<Supplier<Biome>> optional) {
        ...
        if (!optional.isPresent()) {
            ...
            this.biome = () -> (Biome)registry.getOrThrow(Biomes.PLAINS);
        } else {
    ...
    public Biome getBiomeFromSettings() {
        ...
        for (Map.Entry entry : this.structureSettings.structureConfig().entrySet()) {
            builder.addStructureStart(biomeGenerationSettings.withBiomeConfig(STRUCTURE_FEATURES.get(entry.getKey())));
        }
        boolean bl2 = bl = (!this.voidGen || this.biomes.getResourceKey((Object)biome).equals(Optional.of(Biomes.THE_VOID))) && this.decoration;
        if (bl) {
            List list = biomeGenerationSettings.features();
        ...
        for (i = 0; i < blockStates.length; ++i) {
            ...
            this.layers[i] = null;
            builder.addFeature(GenerationStep.Decoration.TOP_LAYER_MODIFICATION, Feature.FILL_LAYER.configured((FeatureConfiguration)new LayerConfiguration(i, blockState)));
        }
    ...
    public static FlatLevelGeneratorSettings getDefault(Registry<Biome> registry) {
        StructureSettings structureSettings = new StructureSettings(Optional.of(StructureSettings.DEFAULT_STRONGHOLD), (Map)Maps.newHashMap((Map)ImmutableMap.of((Object)StructureFeature.VILLAGE, (Object)StructureSettings.DEFAULTS.get((Object)StructureFeature.VILLAGE))));
        FlatLevelGeneratorSettings flatLevelGeneratorSettings = new FlatLevelGeneratorSettings(structureSettings, registry);
        flatLevelGeneratorSettings.biome = () -> (Biome)registry.getOrThrow(Biomes.PLAINS);
