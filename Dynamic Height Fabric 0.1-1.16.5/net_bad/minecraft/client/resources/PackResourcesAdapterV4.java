   ...
   public boolean hasResource(PackType packType, ResourceLocation resourceLocation) {
      if (!"minecraft".equals(resourceLocation.getNamespace())) {
         ...
         if ("textures/misc/enchanted_item_glint.png".equals(string)) {
            ...
            return pair != null && this.pack.hasResource(packType, (ResourceLocation)pair.getSecond())
               ? true
               : this.pack.hasResource(packType, resourceLocation);
         } else {
      ...
      if (!"minecraft".equals(resourceLocation.getNamespace())) {
         ...
         if ("textures/entity/iron_golem/iron_golem.png".equals(string)) {
            ...
            if (SHIELDS.contains(string)) {
               InputStream inputStream = fixPattern(
                  this.pack.getResource(packType, SHIELD_BASE), this.pack.getResource(packType, resourceLocation), 64, 2, 2, 12, 22
               );
               if (inputStream != null) {
                  return inputStream;
               }
               ...
               InputStream inputStream = fixPattern(
                  this.pack.getResource(packType, BANNER_BASE), this.pack.getResource(packType, resourceLocation), 64, 0, 0, 42, 41
               );
               if (inputStream != null) {
                  return inputStream;
               }
               ...
               if (pair != null) {
                  ...
                  InputStream inputStream2 = this.pack.getResource(packType, (ResourceLocation)pair.getSecond());
   ...
   public static InputStream fixPattern(InputStream inputStream, InputStream inputStream2, int i, int j, int k, int l, int m) throws IOException {
      ...
      try (
         ...
         try (NativeImage nativeImage3 = new NativeImage(n, o, true)) {
            ...
            for(int y = k * p; y < m * p; ++y) {
               for(int x = j * p; x < l * p; ++x) {
                  int s = NativeImage.getR(nativeImage2.getPixelRGBA(x, y));
                  int t = nativeImage.getPixelRGBA(x, y);
                  nativeImage3.setPixelRGBA(x, y, NativeImage.combine(s, NativeImage.getB(t), NativeImage.getG(t), NativeImage.getR(t)));
               }
            }

            result = new ByteArrayInputStream(nativeImage3.asByteArray());
         }
      }
