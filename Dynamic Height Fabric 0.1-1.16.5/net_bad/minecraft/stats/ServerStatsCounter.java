...
public class ServerStatsCounter extends StatsCounter {
                           ...
                           statType -> {
                              ...
                              for(String string2xx : compoundTag2xx.getAllKeys()) {
                                 if (compoundTag2xx.contains(string2xx, 99)) {
                                    ...
                                    LOGGER.warn(
                                       "Invalid statistic value in {}: Don't know what {} is for key {}", this.file, compoundTag2xx.get(string2xx), string2xx
                                    );
                                 }
                              }
                           },
   ...
   protected String toJson() {
      ...
      ObjectIterator jsonObject = this.stats.object2IntEntrySet().iterator();

      while(jsonObject.hasNext()) {
         it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<Stat<?>> entry = (it.unimi.dsi.fastutil.objects.Object2IntMap.Entry)jsonObject.next();
      ...
      for(Entry<StatType<?>, JsonObject> entry2 : map.entrySet()) {
         jsonObjectx.add(Registry.STAT_TYPE.getKey(entry2.getKey()).toString(), (JsonElement)entry2.getValue());
      }
