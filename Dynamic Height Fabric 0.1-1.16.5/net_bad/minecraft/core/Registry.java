...
public abstract class Registry<T> implements Codec<T>, Keyable, IdMap<T> {
   ...
   public static final Registry<Codec<? extends BiomeSource>> BIOME_SOURCE = registerSimple(
      BIOME_SOURCE_REGISTRY, Lifecycle.stable(), () -> BiomeSource.CODEC
   );
   ...
   public static <T extends WritableRegistry<?>> void checkRegistry(WritableRegistry<T> writableRegistry) {
      writableRegistry.forEach(writableRegistry2 -> {
         if (writableRegistry2.keySet().isEmpty()) {
            LOGGER.error("Registry '{}' was empty after loading", writableRegistry.getKey(writableRegistry2));
   ...
   private static <T, R extends WritableRegistry<T>> R internalRegister(
      ...
      return (R)writableRegistry2.register((ResourceKey<WritableRegistry<?>>)resourceKey, (WritableRegistry<?>)writableRegistry, lifecycle);
   }
   ...
   public <U> Stream<U> keys(DynamicOps<U> dynamicOps) {
      return this.keySet().stream().map(resourceLocation -> (U)dynamicOps.createString(resourceLocation.toString()));
   }
   ...
   public static <V, T extends V> T register(Registry<V> registry, ResourceLocation resourceLocation, T object) {
      return (T)((WritableRegistry)registry).register(ResourceKey.create(registry.key, resourceLocation), object, Lifecycle.stable());
   }

   public static <V, T extends V> T registerMapping(Registry<V> registry, int i, String string, T object) {
      return (T)((WritableRegistry)registry).registerMapping(i, ResourceKey.create(registry.key, new ResourceLocation(string)), object, Lifecycle.stable());
   }
