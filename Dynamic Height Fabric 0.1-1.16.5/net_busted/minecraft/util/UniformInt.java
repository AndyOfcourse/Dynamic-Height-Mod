   ...
   public static final Codec<UniformInt> CODEC = Codec.either(
         ...
         RecordCodecBuilder.create(
               instance -> instance.group(
                        Codec.INT.fieldOf("base").forGetter(uniformInt -> uniformInt.baseValue),
                        Codec.INT.fieldOf("spread").forGetter(uniformInt -> uniformInt.spread)
                     )
                     .apply(instance, UniformInt::new)
            )
            .comapFlatMap(
               uniformInt -> uniformInt.spread < 0
                     ? DataResult.error("Spread must be non-negative, got: " + uniformInt.spread)
                     : DataResult.success(uniformInt),
               Function.identity()
            )
      )
      .xmap(
         either -> (UniformInt)either.map(UniformInt::fixed, uniformInt -> uniformInt),
