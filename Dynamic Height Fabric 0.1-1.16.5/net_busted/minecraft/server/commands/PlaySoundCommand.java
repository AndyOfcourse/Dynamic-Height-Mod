...
public class PlaySoundCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
      ...
      commandDispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("playsound").requires(commandSourceStack -> commandSourceStack.hasPermission(2)))
            .then(requiredArgumentBuilder)
      );
   }

   private static LiteralArgumentBuilder<CommandSourceStack> source(SoundSource soundSource) {
      return (LiteralArgumentBuilder<CommandSourceStack>)Commands.literal(soundSource.getName())
         .then(
            ((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.players())
                  .executes(
                     commandContext -> playSound(
                           ...
                           soundSource,
                           ((CommandSourceStack)commandContext.getSource()).getPosition(),
                           1.0F,
                           1.0F,
                           0.0F
                        )
      ...
      if (i == 0) {
         ...
         if (collection.size() == 1) {
            commandSourceStack.sendSuccess(
               new TranslatableComponent("commands.playsound.success.single", resourceLocation, collection.iterator().next().getDisplayName()), true
            );
         } else {
