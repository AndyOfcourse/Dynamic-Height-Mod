         ...
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("forceload")
                                 ...
                                 Commands.argument("to", ColumnPosArgument.columnPos())
                                    .executes(
                                       commandContext -> changeForceLoad(
                                             (CommandSourceStack)commandContext.getSource(),
                                             ColumnPosArgument.getColumnPos(commandContext, "from"),
                                             ColumnPosArgument.getColumnPos(commandContext, "to"),
                                             true
                                          )
                                    )
                              )
               ...
               .then(
                  ((LiteralArgumentBuilder)Commands.literal("remove")
                     ...
                     .then(Commands.literal("all").executes(commandContext -> removeAll((CommandSourceStack)commandContext.getSource())))
               ))
            .then(
               ((LiteralArgumentBuilder)Commands.literal("query").executes(commandContext -> listForceLoad((CommandSourceStack)commandContext.getSource())))
                  .then(
                     Commands.argument("pos", ColumnPosArgument.columnPos())
                        .executes(
                           commandContext -> queryForceLoad(
               ...
               if (r == 1) {
                  ...
                  commandSourceStack.sendSuccess(
                     new TranslatableComponent(
                        "commands.forceload." + (bl ? "added" : "removed") + ".multiple", r, resourceKey.location(), chunkPos2, chunkPos3
                     ),
                     true
                  );
               }
