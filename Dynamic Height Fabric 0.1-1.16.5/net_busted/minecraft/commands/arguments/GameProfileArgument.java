...
public class GameProfileArgument implements ArgumentType<GameProfileArgument.Result> {
   ...
   public static Collection<GameProfile> getGameProfiles(CommandContext<CommandSourceStack> commandContext, String string) throws CommandSyntaxException {
      return ((GameProfileArgument.Result)commandContext.getArgument(string, GameProfileArgument.Result.class))
         .getNames((CommandSourceStack)commandContext.getSource());
   }
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> commandContext, SuggestionsBuilder suggestionsBuilder) {
      if (commandContext.getSource() instanceof SharedSuggestionProvider) {
         ...
         return entitySelectorParser.fillSuggestions(
            suggestionsBuilder,
            suggestionsBuilderx -> SharedSuggestionProvider.suggest(
                  ((SharedSuggestionProvider)commandContext.getSource()).getOnlinePlayerNames(), suggestionsBuilderx
               )
         );
      } else {
