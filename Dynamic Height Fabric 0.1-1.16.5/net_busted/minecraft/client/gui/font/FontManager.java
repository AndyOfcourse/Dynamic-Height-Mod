...
public class FontManager implements AutoCloseable {
   ...
   private final PreparableReloadListener reloadListener = new SimplePreparableReloadListener<Map<ResourceLocation, List<GlyphProvider>>>() {
      protected Map<ResourceLocation, List<GlyphProvider>> prepare(ResourceManager resourceManager, ProfilerFiller profilerFiller) {
         ...
         for(ResourceLocation resourceLocation : resourceManager.listResources("font", stringx -> stringx.endsWith(".json"))) {
            ...
            List<GlyphProvider> list = map.computeIfAbsent(
               resourceLocation2, resourceLocationx -> Lists.newArrayList(new GlyphProvider[]{new AllMissingGlyphProvider()})
            );
            ...
            try {
               for(Resource resource : resourceManager.getResources(resourceLocation)) {
                  ...
                  try (
                     ...
                     FontManager.LOGGER
                        .warn(
                           "Unable to load font '{}' in fonts.json in resourcepack: '{}': {}", resourceLocation2, resource.getSourceName(), var54.getMessage()
                        );
                  }
   ...
   public FontManager(TextureManager textureManager) {
      ...
      this.missingFontSet = Util.make(
         new FontSet(textureManager, MISSING_FONT), fontSet -> fontSet.reload(Lists.newArrayList(new GlyphProvider[]{new AllMissingGlyphProvider()}))
      );
   }
   ...
   public Font createFont() {
      return new Font(resourceLocation -> this.fontSets.getOrDefault(this.renames.getOrDefault(resourceLocation, resourceLocation), this.missingFontSet));
   }
