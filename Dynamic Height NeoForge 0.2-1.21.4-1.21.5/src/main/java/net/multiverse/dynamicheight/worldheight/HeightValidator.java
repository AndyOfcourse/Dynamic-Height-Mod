...
public final class HeightValidator {
    ...
    public static int sanitizeMin(int value) {
        int minAllowed = Math.max(DimensionType.MIN_Y, ABS_MIN_Y);
        int maxMin = Math.min(DimensionType.MAX_Y + 1 - SECTION_SIZE, ABS_MAX_Y - SECTION_SIZE);
    ...
    public static int sanitizeMax(int min, int value) {
        ...
        int maxAllowed = Math.min(DimensionType.MAX_Y + 1, ABS_MAX_Y);
